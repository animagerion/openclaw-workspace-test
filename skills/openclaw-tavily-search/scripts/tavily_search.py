...
def tavily_search(query: str, max_results: int, include_answer: bool, search_depth: str, topic: str = "general", time_range: str = None):
    key = load_key()
    if not key:
        raise SystemExit(
...
def main():
    ...
    ap.add_argument(
        ...
        help="Tavily topic: general, news, or finance",
    )
    ap.add_argument(
        "--time-range",
        default=None,
        choices=["day", "week", "month", "year"],
        help="Filter results by recency: day, week, month, or year",
    )
    ap.add_argument(
        "--format",
        default="raw",
